...
<section>
  ...
  <div class="container pt-10 pb-16 flex flex-col justify-start items-stretch gap-6">
    ...
    <form method="GET" id="search" class="max-w-md">
      <label for="default-search" class="text-neutral-500 text-sm font-semibold">Search Job Titles</label>
      <div class="relative">
          ...
          placeholder="Search..." 
          required>
          <button type="submit" class="font-bold text-white absolute right-2.5 bottom-2.5 bg-blue-500 hover:bg-orange-500 transition-all focus:ring-4 focus:outline-none focus:ring-blue-300 rounded text-sm px-4 py-2">Search</button>
      </div>
    </form>
    ...
    <?php if (!empty($jobs)) : ?>
      ...
      <?php foreach ($jobs as $job) : ?>
        ...
        <?php foreach ($employers as $employer) : ?>
          <?php if ($employer['employer_id'] === $job['employer_id']) : ?>
